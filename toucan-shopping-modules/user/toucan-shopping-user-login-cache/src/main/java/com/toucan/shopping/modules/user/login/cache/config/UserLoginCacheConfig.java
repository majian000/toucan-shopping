...
import com.toucan.shopping.modules.user.login.cache.service.impl.UserLoginCacheCustomShardingRedisServiceImpl;
import com.toucan.shopping.modules.user.login.cache.service.impl.UserLoginCacheDefaultRedisServiceImpl;
...
public class UserLoginCacheConfig {
    ...
    public UserLoginCacheService userLoginCacheService() {
        UserLoginCacheService userLoginCacheService = null;

        if("redis".equals(toucan.getModules().getUserLoginCache().getCacheType()))
        {
            //自定义分片
            if("customSharding".equals(toucan.getModules().getUserLoginCache().getRedisType())) {
                userLoginCacheService = new UserLoginCacheCustomShardingRedisServiceImpl();
                //索引和redis缓存操作对象键值对,格式如下{redis索引:{db索引:obj,db索引:obj}}
                Map<String, Map<String, ToucanStringRedisService>> toucanStringRedisServiceMap = ((UserLoginCacheCustomShardingRedisServiceImpl)userLoginCacheService).getToucanStringRedisServiceMap();
                //索引和数据库数量映射键值对
                Map<String, Integer> dbCountMap = ((UserLoginCacheCustomShardingRedisServiceImpl)userLoginCacheService).getDbCountMap();
                if (CollectionUtils.isNotEmpty(toucan.getModules().getUserLoginCache().getCustomShardingRedis().getLoginCacheRedisList())) {
                    for (UserLoginRedis userLoginRedis : toucan.getModules().getUserLoginCache().getCustomShardingRedis().getLoginCacheRedisList()) {
                ...
                //默认官方配置
            }else if("default".equals(toucan.getModules().getUserLoginCache().getRedisType()))
            {
                userLoginCacheService = new UserLoginCacheDefaultRedisServiceImpl();
                ToucanStringRedisService toucanStringRedisService = new ToucanStringRedisServiceImpl();
                ((ToucanStringRedisServiceImpl) toucanStringRedisService).setRedisTemplate(createToucanRedisTemplate(toucan.getModules().getUserLoginCache().getDefaultRedis()));
                ((UserLoginCacheDefaultRedisServiceImpl)userLoginCacheService).setToucanStringRedisService(toucanStringRedisService);
            }
        }
    ...
    public RedisConnectionFactory toucanRedisConnectionFactory(UserLoginRedis userLoginRedis) {
        ...
        try {
            ...
            if ("cluster".equals(userLoginRedis.getSelect())) {
                log.info(" 初始化犀鸟用户中心-用户登录redis模块 加载配置 toucan.modules.userLoginCache.defaultRedis.hosts:{}",userLoginRedis.getHosts());
                log.info(" 初始化犀鸟用户中心-用户登录redis模块 加载配置 toucan.modules.userLoginCache.defaultRedis.maxRedirects:{}",userLoginRedis.getMaxRedirects());
                ...
                log.info(" 初始化犀鸟用户中心-用户登录redis模块 加载配置 toucan.modules.userLoginCache.defaultRedis.host:{}",userLoginRedis.getHost());
                log.info(" 初始化犀鸟用户中心-用户登录redis模块 加载配置 toucan.modules.userLoginCache.defaultRedis.port:{}",userLoginRedis.getPort());
                log.info(" 初始化犀鸟用户中心-用户登录redis模块 加载配置 toucan.modules.userLoginCache.defaultRedis.database:{}",userLoginRedis.getDatabase());
                //单机模式
