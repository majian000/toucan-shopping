...
function bindRowEvents()
{
    ...
    bindDelEvent();
    bindModifySkuEvent();
    bindModifyFreightTemplateEvent();
}
...
{
    //预览事件
    $(".modifySkuRow").unbind("click");
    //SKU信息
    $(".modifySkuRow").bind("click", function () {
...
/*运费模板分页参数*/
var freightTemplatePagegizationConfigObject={
    obj_box:'.freightTeampltePageToolbar',//翻页容器
    total_item:1,//条目总数
    per_num:10,//每页条目数
    current_page:1//当前页
};
...
//修改运费模板
function bindModifyFreightTemplateEvent()
{

    //预览事件
    $(".modifyFreightTemplateRow").unbind("click");
    //SKU信息
    $(".modifyFreightTemplateRow").bind("click", function () {
        var shopPorudctId = $(this).attr("attr-id");
        var ftAttrId = $(this).attr("ft-attr-id");
        openSelectFreightTemplateDialog(shopPorudctId,ftAttrId);
        //查询运费模板列表
 ...
 * 打开选择运费模板对话框
 */
function openSelectFreightTemplateDialog(shopPorudctId,freightTempalteId)
{

    var freightTemplateTableHtml="  <div class=\"freightTeampltePageToolbar\" style='margin-top:2%'>\n" +
        "                    <table id=\"freightTemplateTable\" class=\"freezeTable\" border=\"1\" width=\"900\">\n" +
        "                        <tbody id=\"freightTemplateTableBody\">\n" +
        "                        </tbody>\n" +
        "                    </table>\n" +
        "                </div>";
    freightTemplateTableHtml+="<input type=\"hidden\" class=\"currentShopProductId\" value=\""+shopPorudctId+"\"  />";
    freightTemplateTableHtml+="<input type=\"hidden\" class=\"currentFreightTemplateId\" value=\""+freightTempalteId+"\"  />";

    g_selectFreightTemplateDialogHandler = layer.open({
        type: 1,
        title:"选择运费模板",
        area: ['55%', '60%'], //宽高
...
function initFreightTemplatePagination()
{
    ...
    $(".freightTeampltePageToolbar").html("<table id=\"freightTemplateTable\" class=\"freezeTable\" border=\"1\" width=\"900\">\n" +
        "                        <tbody id=\"freightTemplateTableBody\">\n" +
        "                        </tbody>\n" +
        "                    </table>");
...
{
    var currentFreightTemplateId=$(".currentFreightTemplateId").val();
    var tableHtml="";
    tableHtml+=" <tr class=\"tabTh\">\n" +
        "                            <td style=\"width:50px;\" >序号</td>\n" +
    ...
    if(pageResult!=null&&pageResult.list!=null&&pageResult.list.length>0)
    {
        for(var i=0;i<pageResult.list.length;i++)
        {
            ...
            tableHtml+=    "                            <td><div class=\"tabTdWrap\">"+row.createDate+"</div></td>\n" ;
            tableHtml+=    "                            <td><div class=\"tabTdWrap\">" ;
            if(currentFreightTemplateId!=row.id) {
                tableHtml += "   &nbsp;<a attr-id=\"" + row.id + "\" attr-name='" + row.name + "' class=\"freightTemplateSelectRow\" style=\"color:blue;cursor: pointer;\">选择</a>\n";
            }
            tableHtml+=    "</div></td>\n" ;
            tableHtml+=    "                        </tr>";
        }
    ...
    bindFreightTemplateRowEvent();
}

function bindFreightTemplateRowEvent()
{

    $(".freightTemplateSelectRow").unbind("click");
    $(".freightTemplateSelectRow").bind("click", function () {
        var attrId = $(this).attr("attr-id");
        var currentShopProductId=$(".currentShopProductId").val();
        var dialogZIndex = layer.zIndex;
        layer.confirm("确定修改运费模板?", {
            btn: ['确定','关闭'], //按钮
            title:'提示信息',
            zIndex: dialogZIndex+1
        }, function(index) {
