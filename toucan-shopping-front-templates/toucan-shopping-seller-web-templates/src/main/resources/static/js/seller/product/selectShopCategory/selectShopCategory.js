...
function intRootShopCategory() {
	...
	for (var i=0; i<shop_category_list.length; i++) {
		areaShopCategoryCont += '<li onClick="selectShopCategoryLevel1(' + i + ');"><a href="javascript:void(0)">' + shop_category_list[i].name + '</a></li>';
	}
...
function selectShopCategoryLevel1(p) {
	...
	for (var j=0; j<shop_category_list[p].children.length; j++) {
		areaShopCategoryCont += '<li onClick="selectShopCategoryLevel2(' + p + ',' + j + ');"><a href="javascript:void(0)">' + shop_category_list[p].children[j].name + '</a></li>';
	}
