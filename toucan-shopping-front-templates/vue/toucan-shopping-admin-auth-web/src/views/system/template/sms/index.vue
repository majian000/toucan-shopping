  ...
  <div class="sms-template-management">
    ...
    <el-card shadow="never" class="search-card">
      <el-form :model="searchForm" inline>
        ...
        <el-form-item>
          <el-button type="primary" :icon="Search" v-permission="'system:smsTemplate:list'" @click="handleSearch">搜索</el-button>
          <el-button :icon="Refresh" v-permission="'system:smsTemplate:list'" @click="handleReset">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never" class="table-card">
      <div class="toolbar">
        <el-button type="primary" :icon="Plus" v-permission="'system:smsTemplate:add'" @click="handleAdd">新增模板</el-button>
        <el-button type="success" :icon="Refresh" v-permission="'system:smsTemplate:list'" @click="handleRefreshCache">刷新缓存</el-button>
      </div>

      <el-table :data="templates" border stripe v-loading="loading" style="width:100%">
        <el-table-column type="index" label="序号" width="60" align="center" />
        <el-table-column prop="name" label="模板名称" width="180" />
        <el-table-column prop="code" label="模板编码" width="180" show-overflow-tooltip />
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            ...
            <el-button type="primary" link size="small" :icon="Edit" v-permission="'system:smsTemplate:edit'" @click="handleEdit(row)">编辑</el-button>
            <el-button type="danger" link size="small" :icon="Delete" v-permission="'system:smsTemplate:delete'" @click="handleDelete(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
        ...
        <el-form-item label="模板内容" prop="content">
          <el-input
            ...
            type="textarea"
            :rows="6"
            placeholder="请输入短信模板内容，变量可使用 ${变量名} 占位"
            maxlength="2000"
            show-word-limit
          />
        </el-form-item>
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="模板内容" :span="2">
          <el-input :model-value="viewData.content" type="textarea" :rows="6" readonly />
        </el-descriptions-item>
...
import { listSmsTemplate, addSmsTemplate, updateSmsTemplate, delSmsTemplate, refreshSmsTemplateCache } from '@/api/system/smsTemplate'
...
async function fetchData() {
  ...
  try {
    ...
    const res = await listSmsTemplate(params)
...
async function handleRefreshCache() {
  try {
    const res = await refreshSmsTemplateCache()
...
const dialogTitle = computed(() => isEdit.value ? '编辑短信模板' : '新增短信模板')
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateSmsTemplate({ id: editingId.value, ...payload })
      ElMessage.success('编辑成功')
    } else {
      await addSmsTemplate(payload)
