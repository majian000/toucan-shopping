<template>
  <div class="config-category-management">
    ...
    <el-card shadow="never" class="search-card">
      <el-form :model="searchForm" inline>
        ...
        <el-form-item>
          <el-button type="primary" :icon="Search" v-permission="'system:configCategory:list'" @click="handleSearch">搜索</el-button>
          <el-button :icon="Refresh" v-permission="'system:configCategory:list'" @click="handleReset">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never" class="table-card">
      <div class="toolbar">
        <el-button type="primary" :icon="Plus" v-permission="'system:configCategory:add'" @click="handleAdd">新增分类</el-button>
      </div>

      <el-table :data="categories" border stripe v-loading="loading" style="width:100%">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" link size="small" :icon="Edit" v-permission="'system:configCategory:edit'" @click="handleEdit(row)">编辑</el-button>
            <el-button type="danger" link size="small" :icon="Delete" v-permission="'system:configCategory:delete'" @click="handleDelete(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
        ...
        <el-form-item label="分类编码" prop="code">
          <el-input v-model="formData.code" placeholder="请输入分类编码（唯一）" maxlength="50" />
        </el-form-item>
...
import { listConfigCategory, addConfigCategory, updateConfigCategory, delConfigCategory } from '@/api/system/configCategory'
...
async function fetchData() {
  ...
  try {
    ...
    const res = await listConfigCategory(params)
    categories.value = res.data.rows || res.data.list || []
...
const formData = reactive({ name: '', code: '', sort: 0, remark: '', enableStatus: 1 })
...
function handleDelete(row) {
  ElMessageBox.confirm(`确认删除分类"${row.name}"吗？删除后不可恢复。`, '删除确认', {
    confirmButtonText: '确定删除',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await delConfigCategory(row.id)
