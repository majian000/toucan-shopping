<template>
  <div class="true-name-approve-management">
    <h2 class="page-title">实名审核</h2>
    <el-card shadow="never" class="search-card">
      <el-form :model="searchForm" :inline="true">
        <el-form-item label="真实姓名">
          <el-input v-model="searchForm.trueName" placeholder="请输入真实姓名" clearable style="width:160px" />
        </el-form-item>
    ...
    <el-card shadow="never" class="table-card">
      <el-table :data="tableData" border stripe v-loading="loading" row-key="id">
        ...
        <el-table-column label="证件照正面" width="110" align="center">
          <template #default="{ row }">
            <el-image v-if="row.httpIdcardImg1" :src="row.httpIdcardImg1" :preview-src-list="[row.httpIdcardImg1]" preview-teleported fit="cover" style="width:60px;height:40px;cursor:zoom-in" />
          </template>
        </el-table-column>
        <el-table-column label="证件照背面" width="110" align="center">
          <template #default="{ row }">
            <el-image v-if="row.httpIdcardImg2" :src="row.httpIdcardImg2" :preview-src-list="[row.httpIdcardImg2]" preview-teleported fit="cover" style="width:60px;height:40px;cursor:zoom-in" />
          </template>
        </el-table-column>
        <el-table-column prop="idcardType" label="证件类型" width="100" align="center">
          <template #default="{ row }">
            {{ idcardTypeText(row.idcardType) }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="rejectText" label="驳回原因" width="160" show-overflow-tooltip />
...
import { listTrueNameApprove, passTrueNameApprove, rejectTrueNameApprove } from '@/api/user/trueNameApprove'

const searchForm = reactive({ trueName: '', idCard: '', approveStatus: '' })

function handleSearch() { pagination.page = 1; loadTableData() }
function handleReset() { searchForm.trueName = ''; searchForm.idCard = ''; searchForm.approveStatus = '' }
...
async function loadTableData() {
  loading.value = true
  try {
    const res = await listTrueNameApprove(buildSearchParams())
    tableData.value = res.data || []
    pagination.total = res.count || 0
  } catch { } finally { loading.value = false }
}

function idcardTypeText(type) {
  if (type === 1 || type === '1') return '身份证'
  if (type === 2 || type === '2') return '护照'
  if (type === 3 || type === '3') return '海外'
  return '-'
}
...
function handlePass(row) {
  ElMessageBox.confirm('确定审核通过该实名认证吗？', '确认', { confirmButtonText: '确定通过', cancelButtonText: '取消', type: 'warning' })
    .then(async () => {
      try { await passTrueNameApprove(row.id, row.userMainId); ElMessage.success('审核通过'); loadTableData() } catch { }
    }).catch(() => {})
}
...
async function handleRejectSubmit() {
  ...
  try {
    await rejectTrueNameApprove({ ...rejectForm })
