<template>
  <div class="shop-banner-management">
    ...
    <el-card shadow="never" class="table-card">
      <el-table :data="tableData" border stripe v-loading="loading" row-key="id">
        ...
        <el-table-column prop="clickPath" label="点击路径" width="180" show-overflow-tooltip />
    ...
    <el-dialog v-model="detailVisible" title="查看轮播图" width="650px" :close-on-click-modal="false" destroy-on-close>
      <el-descriptions v-if="detailInfo" :column="2" border label-width="110px">
        ...
        <el-descriptions-item label="预览图" :span="2">
          <el-image v-if="detailInfo.httpImgPath" :src="detailInfo.httpImgPath" :preview-src-list="[detailInfo.httpImgPath]" preview-teleported fit="contain" style="width:180px;height:160px" />
        </el-descriptions-item>
        <el-descriptions-item label="位置">{{ formatPosition(detailInfo.position) }}</el-descriptions-item>
        <el-descriptions-item label="排序">{{ detailInfo.bannerSort }}</el-descriptions-item>
    ...
    <el-dialog v-model="dialogVisible" title="编辑轮播图" width="620px" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="110px">
        ...
        <el-form-item label="显示位置" prop="position">
          <el-select v-model="formData.position" style="width:100%">
            <el-option label="PC首页" value="PC_INDEX" />
        ...
        <el-form-item label="开始展示时间" prop="startShowDateString">
          <el-date-picker v-model="formData.startShowDateString" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择" style="width:100%" />
        </el-form-item>
        <el-form-item label="结束展示时间" prop="endShowDateString">
          <el-date-picker v-model="formData.endShowDateString" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择" style="width:100%" />
        </el-form-item>
        ...
        <el-form-item label="排序">
          <el-input-number v-model="formData.bannerSort" :min="0" style="width:100%" />
        </el-form-item>
...
function handleReset() { searchForm.title = ''; searchForm.shopId = ''; searchForm.startShowDate = ''; searchForm.endShowDate = ''; handleSearch() }
...
async function loadTableData() {
  loading.value = true
  try {
    const res = await listShopBanner(buildSearchParams())
...
function formatPosition(position) {
  if (position === 'PC_INDEX') return 'PC首页'
  if (position === 'H5_INDEX') return 'H5首页'
  return position
}
