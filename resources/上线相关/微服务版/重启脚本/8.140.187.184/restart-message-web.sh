...
SERVICE="toucan-shopping-message-web"
DEPLOY_DIR="/usr/toucan_shopping"
LOG_DIR="/usr/toucan_shopping/logs"
mkdir -p ${LOG_DIR}

echo "===== 重启 ${SERVICE} (消息WEB端) ====="

# ===== 1. 停服 =====
# 用JAR包名精确匹配，避免 admin 误匹配 admin-auth / admin-auth-web 等
OLD_PID=$(ps aux | grep "toucan-shopping-message-web-[0-9]" | grep -v grep | awk '{print $2}')
if [ -n "${OLD_PID}" ]; then
...
nohup java -Xmx200m  -XX:+UseG1GC -XX:SoftRefLRUPolicyMSPerMB=0 -Djava.awt.headless=true -Dfile.encoding=UTF-8 -XX:ParallelGCThreads=4 -XX:SurvivorRatio=8 -XX:TargetSurvivorRatio=80 -XX:MaxTenuringThreshold=15 -XX:+HeapDumpOnOutOfMemoryError -Xlog:gc*:file=./gc.log:time,level,tags:filecount=5,filesize=10M \
    -jar toucan-shopping-message-web-1.0-SNAPSHOT.jar \
...
if ps -p ${NEW_PID} > /dev/null 2>&1; then
    ...
    echo "[3/3] 启动失败! 查看日志: tail -20 ${LOG_DIR}/message-web.log"
    tail -20 "${LOG_DIR}/message-web.log" 2>/dev/null || echo "(日志为空)"
    exit 1
fi
echo ""
