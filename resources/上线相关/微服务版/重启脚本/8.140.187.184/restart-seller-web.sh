...
OLD_PID=$(ps aux | grep "toucan-shopping-seller-web-[0-9]" | grep -v grep | awk '{print $2}')
if [ -n "${OLD_PID}" ]; then
...
if ps -p ${NEW_PID} > /dev/null 2>&1; then
    ...
    echo "[3/3] 启动失败! 查看日志: tail -20 ${LOG_DIR}/seller-web.log"
    tail -20 "${LOG_DIR}/seller-web.log" 2>/dev/null || echo "(日志为空)"
    exit 1
fi
echo ""
