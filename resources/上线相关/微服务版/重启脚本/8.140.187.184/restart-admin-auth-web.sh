...
OLD_PID=$(ps aux | grep "toucan-shopping-admin-auth-web-[0-9]" | grep -v grep | awk '{print $2}')
if [ -n "${OLD_PID}" ]; then
...
nohup java -Xmx300m  -XX:+UseG1GC -XX:SoftRefLRUPolicyMSPerMB=0 -Djava.awt.headless=true -Dfile.encoding=UTF-8 -XX:ParallelGCThreads=4 -XX:SurvivorRatio=8 -XX:TargetSurvivorRatio=80 -XX:MaxTenuringThreshold=15 -XX:+HeapDumpOnOutOfMemoryError -Xlog:gc*:file=./gc.log:time,level,tags:filecount=5,filesize=10M \
    -jar toucan-shopping-admin-auth-web-1.0-SNAPSHOT.jar \
    ...
    > ${LOG_DIR}/admin-auth-web.log 2>&1 &
