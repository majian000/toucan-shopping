...
SERVICE="toucan-shopping-api-monitor"
DEPLOY_DIR="/usr/toucan_shopping"
LOG_DIR="/usr/toucan_shopping/logs"
mkdir -p ${LOG_DIR}

echo "===== 重启 ${SERVICE} (接口监控服务) ====="

# ===== 1. 停服 =====
# 用JAR包名精确匹配，避免 admin 误匹配 admin-auth / admin-auth-web 等
OLD_PID=$(ps aux | grep "toucan-shopping-api-monitor-[0-9]" | grep -v grep | awk '{print $2}')
if [ -n "${OLD_PID}" ]; then
...
if ps -p ${NEW_PID} > /dev/null 2>&1; then
    ...
    echo "[3/3] 启动失败! 查看日志: tail -20 ${LOG_DIR}/api-monitor.log"
    tail -20 "${LOG_DIR}/api-monitor.log" 2>/dev/null || echo "(日志为空)"
    exit 1
fi
echo ""
